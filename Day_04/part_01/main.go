...
func parse() []string {
	...
	data, dataErr := os.Open("_full.txt")
	check(dataErr)
...
func processing(xmasSlice [][]string) int {
	...
	fmt.Println(xmas, " | ", matrixSize, " | ", sliceSize)

	for i := 0; i < matrixSize; i++ {
		for j := 0; j < sliceSize; j++ {

			count += search(xmasSlice, i, j, xmas)
...
func search(grid [][]string, row int, column int, xmas string) int {
	mSize := len(grid)
	sSize := len(grid)
	count := 0

	if grid[row][column] != string(xmas[0]) {
		return 0
	}
	...
	xCoords := []int{-1, -1, -1, 0, 0, 1, 1, 1}
	yCoords := []int{-1, 0, 1, -1, 1, -1, 0, 1}
	...
	for dir := 0; dir < 8; dir++ {
		...
		for charIndex = 1; charIndex < xmasLen; charIndex++ {
			...
			if grid[currX][currY] != string(xmas[charIndex]) {
				break
			}
